          ...
          <tbody className="divide-y divide-gray-100">
            {agents.map((agent) => (
              <tr key={agent.id} className="hover:bg-gray-50">
                ...
                <td className="p-4 text-gray-500">{agent.contact}</td>
                <td className="p-4 text-gray-500 flex items-center gap-1">
                  <Link size={14} className="text-gray-400" />
                  {agent.agentId}
                </td>
                <td className="p-4 text-gray-600">{agent.plan}</td>
                <td className="p-4 text-gray-600">{agent.credits}</td>
                <td className="p-4 text-gray-600">{agent.renewal}</td>
                <td className="p-4">
                  <span className="w-8 h-8 rounded-full border border-orange-200 text-orange-600 flex items-center justify-center text-xs font-medium bg-orange-50">
                ...
                <td className="p-4 text-gray-500">{agent.lastConnected}</td>
                <td className="p-4 text-gray-500">{agent.lastTried}</td>
                <td className="p-4">
                ...
                <td className="p-4">
                  <div className="flex items-center justify-between text-gray-600 bg-gray-50 px-2 py-1 rounded border border-gray-200">
                    {agent.kam}
                    <ChevronDown size={14} className="text-gray-400" />
                  </div>
                </td>
                <td className="p-4">
                   <div className="flex items-center justify-between text-gray-600 bg-green-50 px-2 py-1 rounded border border-green-200">
                    {agent.agentStatus}
                    <ChevronDown size={14} className="text-gray-400" />
                  </div>
                </td>
                <td className="p-4">
                  <div className="flex items-center gap-2">
                    <button className="p-1.5 text-gray-500 bg-gray-100 rounded hover:bg-gray-200">
                      <Phone size={16} />
                    </button>
                    <button className="p-1.5 text-gray-500 bg-gray-100 rounded hover:bg-gray-200">
                      <Trash2 size={16} />
